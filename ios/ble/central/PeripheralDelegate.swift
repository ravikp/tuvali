...
extension Central: CBPeripheralDelegate {
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        for service in peripheralServices where UUIDConstants.SERVICE_UUID == service.uuid {
            peripheral.discoverCharacteristics(UUIDConstants.CharacteristicIds.allCases.map{CBUUID(string: $0.rawValue)}, for: service)
        }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        ...
        for characteristic in serviceCharacteristics {
            ...
            if characteristic.uuid == UUIDConstants.NetworkCharNums.SUBMIT_RESPONSE_CHAR_UUID {
            ...
            if characteristic.uuid == UUIDConstants.NetworkCharNums.TRANSFER_REPORT_RESPONSE_CHAR_UUID ||
                characteristic.uuid == UUIDConstants.NetworkCharNums.VERIFICATION_STATUS_CHAR_UUID || characteristic.uuid == UUIDConstants.NetworkCharNums.DISCONNECT_CHAR_UUID
            {
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        } else if characteristic.uuid == UUIDConstants.NetworkCharNums.DISCONNECT_CHAR_UUID {
    ...
    func peripheral(_ peripheral: CBPeripheral, didWriteValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        if characteristic.uuid == UUIDConstants.NetworkCharNums.IDENTIFY_REQUEST_CHAR_UUID {
        ...
        } else if characteristic.uuid == UUIDConstants.NetworkCharNums.RESPONSE_SIZE_CHAR_UUID {
        ...
        } else if characteristic.uuid == UUIDConstants.NetworkCharNums.SUBMIT_RESPONSE_CHAR_UUID {
